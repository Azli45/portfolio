...
function initChatbot(){
  ...
  const quickPrompts = [
    { label:"Tell me about projects", key:"projects" },
    { label:"Why hire her?", key:"hire" },
    { label:"Show certifications", key:"certifications" },
    { label:"Explain the warehouse", key:"warehouse" },
    { label:"Contact info", key:"contact" }
  ];
  quick.innerHTML = quickPrompts.map(q => `<button data-key="${q.key}">${q.label}</button>`).join("");

  let opened = false;
  fab.addEventListener("click", () => {
    opened = !opened;
    panel.hidden = !opened;
    fab.setAttribute("aria-expanded", opened);
    iconOpen.style.display = opened ? "none" : "inline";
    iconClose.style.display = opened ? "inline" : "none";
    if(opened && body.children.length === 0){
      addMessage("bot", "Hi, I'm AzliGPT — ask me anything about Azli's projects, skills, or why you should hire her.");
    }
  });
  ...
  function respond(key){
    setTimeout(() => {
      const fn = CHAT_KB[key] || CHAT_KB.default;
      addMessage("bot", fn());
    }, 350);
  }
  ...
  function classifyIntent(text){
    ...
    if(t.includes("warehouse") || t.includes("architecture") || t.includes("pipeline")) return "warehouse";
    if(t.includes("hire") || t.includes("why you") || t.includes("why should")) return "hire";
    if(t.includes("certif")) return "certifications";
    if(t.includes("skill")) return "skills";
    if(t.includes("experience") || t.includes("ambassador") || t.includes("leadership")) return "experience";
    if(t.includes("resume") || t.includes("cv")) return "resume";
    if(t.includes("contact") || t.includes("email") || t.includes("phone") || t.includes("reach")) return "contact";
    if(t.includes("cgpa") || t.includes("gpa")) return "cgpa";
    return "default";
  }
}
